...
normalLocOrder <- function (y, m = 1, lambdas, graphtype = "MNN", K = NULL, sigma = NULL, arbSigma = TRUE, ...) {
  ...
  if (!is.null(K) && K == 1) stop("Error: K must be at least 2")
  ...
  else maxadmm <- input[["maxadmm"]]

  if (is.null(input[["maxNR"]])) maxNR <- 100
  else maxNR <- input[["maxNR"]]
...
#'    # Requires ggplot2.
#'    plot(out, gg=TRUE, eta=FALSE)
tLocOrder <- function (y, m = 1, lambdas, df = NULL, graphtype = "MNN", K = NULL, sigma = NULL, arbSigma = TRUE, ...) {
  input <- list(...)
  ...
  # df is the named parameter (degrees of freedom).
  # If not provided (NULL), default to 1 (Cauchy) per component after K is known.
  # If provided as a scalar, expand to a vector of length K after K is known.
  # NOTE: do NOT read df from input[["df"]] here, because df is a named parameter
  # and will never appear in ...; the old code using input[["df"]] was a bug.
  .expand_df <- function(df_val, K_val) {
    if (is.null(df_val)) return(rep(1, K_val))
    if (length(df_val) == 1) return(rep(df_val, K_val))
    df_val
  }
...
poissonOrder <- function (y, lambdas, K = NULL, ...) {
  ...
  out <- .myEm(matrix(y, ncol=1), 1L, 1L, matrix(theta, nrow=1), diag(1), pii, FALSE, -1L, 5L,
              maxadmm, maxNR, C, a, .penCode(penalty), lambdas, epsilon, delta, maxMem, verbose, FALSE)
...
exponentialOrder <- function (y, lambdas, K = NULL, theta, ...) {
  ...
  if (!is.null(theta)) {
    out <- .myEm(as.matrix(y, ncol=1), 1L, 1L, matrix(theta, nrow=1), diag(1),
                pii, FALSE, -1L, 6L, maxadmm, maxNR, C, a, .penCode(penalty),
                lambdas, epsilon, delta, maxMem, verbose, FALSE)
